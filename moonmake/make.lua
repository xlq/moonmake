...
--[[
function builder:visit_node(node)
    if not node._visited then
        local node_name = node.target
        self:debug("Visiting: %s", node_name)
        node._visited = true
        ...
        for i = 1, #depends do
            ...
            if self:visit_node(dep) then
                ...
                if not dep_mtime then
                    -- dep does not exist
                    self:debug("Dep `%s' does not exist (yet)", dep.target)
                    need_update = true
        ...
        if node_command then
            -- Form command line
            --local cmdline = subprocess.form_cmdline(node_command)
            local oldcmdline = self.state.commands[node_name]
            -- Has command line changed?
            if oldcmdline and not util.compare(oldcmdline, node_command) then
                self:debug("Command for %s has changed", node_name)
                need_update = true
            end
            self.state.commands[node_name] = node_command
            if need_update and self.opts.question then os.exit(1) end
            -- Handle dynamic dependencies
            ...
            if need_update then
                ...
                if not self.opts.dry_run then
                    local proc = subprocess.popen(node_command)
                    local returncode = proc:wait()
        ...
        self:debug("Re-visiting: %s", node.target)
...
--[[
    ...
    for _, node in ipairs(S) do
        self:debug("Initial visit: %s", node.target)
